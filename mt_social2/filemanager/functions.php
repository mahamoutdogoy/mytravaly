...
function is_audio_extenstion($extension){
    return in_array(strtolower($extension), array('mp3'));
}
...
function Dirtree($path, $name = "Upload to: Home", $prefix = "") {
	...
	$list = '';
	if(strpos($path, 'uploads/' . $_SESSION['active_folder'] . '/' . USER_FOLDER . '/') !== FALSE) :
	...
	if(count($dircont) > 0){
		foreach($dircont as $file){
			if(is_file($path . $file)){
				...
				array_push($sortedData,  array('is_file'=>false, 'name'=>$file, 'path'=>$path . $file . '/', 'i'=>$count));
			}
		}
	}
	return $sortedData;
...
function DoUpload($field = 'userfile'){
	...
	$current_folder = LIBRARY_FOLDER_PATH . USER_FOLDER . '/';
...
function recentImageArray()
{
	$return_image_array = [];

	$images = scandir(LIBRARY_FOLDER_PATH . USER_FOLDER, SCANDIR_SORT_DESCENDING);
	$images = array_diff($images, ['.', '..']);

	$count = 0;
	$filectime_array = [];
	foreach($images as $image) :
		$filepath = LIBRARY_FOLDER_PATH . USER_FOLDER . '/' . $image;
		$filectime = filectime($filepath);
		if(in_array($filectime,$filectime_array)) :
			$count_values = array_count_values($filectime_array);
			$repeat_no = $count_values[$filectime];
			$filectime_array[] = $filectime;
			$filectime = $filectime + $repeat_no;
		else :
			$filectime_array[] = $filectime;
		endif;	
		$return_image_array[$filectime] = FOLDER_URL . $_SESSION['active_folder'] . '/' . USER_FOLDER . '/' . $image;
	endforeach;	

	ksort($return_image_array);

	$preserved = array_reverse($return_image_array, true);
	$preserved = array_slice($preserved, 0, RECENT_IMAGE_NO);
