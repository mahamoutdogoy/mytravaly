...
$email = $_POST['email'];
	...
	$gender      = $_POST['gender'];
	$query = "INSERT INTO reservation(property_id,room_id,inventory_id,folio_id,guestName,gender,email,phoneNo,check_in,check_out,meal_plan,noOfAdults,noOfChildren,price,total,status) VALUES('$property_id','$room_id','$inventory_id','','$name','$gender','$email','$number','$check_in','$check_out','$meal_plan','$people','$child','$full_price','$total_price','')";
...
Reservation Management</p></td>
		...
		if($mail == 1){
			$book= '<div class="w3-panel w3-green w3-round">
				  <h6 class="text-success">Success!Go ahead and proceed payment</h6>
     ...
     <body class="bg" style="background-image: url('payment7.jpg');height: 100%;background-position: center;background-repeat: no-repeat;background-size: cover;">
     	...
     	<div class="container mw-100" style="background-color: #f7f2f9">
     		<div class="col-sm-4  pt-0 pb-0 pr-1 mb-4" style="border-radius:50px;float: right;background-color: #FF6F56;">
     			...
     			<p><i class="fas fa-money-check-alt" style="color: blue"></i>&nbsp;&nbsp;Reservation Fees:<?php $reservationfee = $_SESSION['full_price']+$_SESSION['tax']; echo $reservationfee; ?></p>
	...
	<form method="POST" style="border-bottom-right-radius: 10px;border-bottom: 10px solid #FF6F71;">
		<div class=" row ">
			<p class=" col-sm-6">
				...
				<input type="text"  name="first_name" class="w-100" required="" value="<?php if(isset($_POST['gender'])){
			...
			<p class="col-sm-6 " >
				...
				<input type="text" name="last_name" id="" class="w-100" required="" value="<?php if(isset($_POST['gender'])){
		...
		<div class=" row">
			...
			<p class=" col-sm-6 " >
				<strong>Email*</strong>
				<input type="email" name="email" id="email1" class="form-control w-100" required="" value="<?php if(isset($_POST['gender'])){
					echo $_SESSION['email'];
				} ?>">
			</p>
			...
			<p class="col-sm-6 ">
				<strong>Phone*</strong>
				<input type="text" name="number" class="w-100" required="" value="<?php if(isset($_POST['gender'])){
		...
		<div class="row">
			...
			<div class="col-sm-4">
				...
				<input type="radio" name="gender" value="mal" onchange="this.form.submit()">Male&nbsp;
				<input type="radio" name="gender" value="female" onchange="this.form.submit()">Female&nbsp;
				<input type="radio" name="gender" value="other" onchange="this.form.submit()">Other
				
			</div>		
		</div>
		<div class="row">
		<div class="col-sm-2"></div><?php echo $book; ?>
		</div>
		</form>
		<form action="">
			<div class="row">
