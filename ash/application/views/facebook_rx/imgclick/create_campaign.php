	...
	<div class="col-xs-12 padding-20">
		<div class="box box-primary">
			<div class="box-header ui-sortable-handle padding-20" style="cursor: move;">
				...
				<h3 class="box-title"><?php echo $this->lang->line("create new campaign"); ?></h3>
				<!-- tools box -->
			...
			<div class="box-body padding-20">
				<form action="#" enctype="multipart/form-data" id="auto_poster_form" method="post">
					...
					<div class="form-group col-xs-12 col-md-6" style="padding:0 5px 0 0;">
						...
						<input type="input" class="form-control"  name="link" id="link" placeholder="<?php echo $this->lang->line('Paste a link where you want to take people on image click'); ?>">
					</div>

					<div class="form-group col-xs-12 col-md-6" style="padding:0 0 0 5px;">
						<label><?php echo $this->lang->line("title"); ?> *</label>
						<input type="input" class="form-control"  name="title" id="title" placeholder="<?php echo $this->lang->line('Any custom title for the link'); ?>">
					</div>


					<div class="form-group col-xs-12 col-md-6" style="padding:0 5px 0 0;">
						<label><?php echo $this->lang->line("description"); ?></label>
						<textarea class="form-control" name="description" id="description" placeholder="<?php echo $this->lang->line('Any custom description for the link'); ?>"></textarea>
					</div>


					<div class="form-group col-xs-12 col-md-6" style="padding:0 0 0 5px;">
						<label><?php echo $this->lang->line("post content"); ?></label>
						<textarea class="form-control" name="message" id="message" placeholder="<?php echo $this->lang->line('Any content to dispaly alongside clickable image ( only works for faceboook, pinterest & linkedin )'); ?>"></textarea>
					</div>				
					...
					<div id="image_block" class="well clearfix">
						<div class="form-group col-xs-12 col-md-7" style="padding:0 5px 0 0">
							...
							<input class="form-control" style="max-width:462.5px;" name="image_url" id="image_url" type="text" placeholder="<?php echo $this->lang->line('Paste image URL or upload new'); ?>" > 
					...
					<div class="form-group">
						...
						<div class="input-group">
						  <?php $protocol=isset($default_redirect_domain_list[0]["protocol"]) ? $default_redirect_domain_list[0]["protocol"] : "http://";?>
						  ...
						  <input type="text" class="form-control hidden" style="height: 42px !important"  name="subdomain" value="<?php echo $this->session->userdata('default_subdomain');?>" id="subdomain" placeholder="<?php echo $this->lang->line('sub-domain'); ?> <?php echo $this->lang->line('(optional)'); ?>">
								...
								echo "<select class='form-control' name='action_controller_url' id='action_controller_url' style='border:none;'>";
									if(!empty($default_redirect_domain_list) || $this->config->item('use_app_domain_as_action_controller_link')==TRUE)
									{
										...
										if($this->config->item('use_app_domain_as_action_controller_link')==TRUE)
										{
											...
											echo "<option data-type='default' data-protocol='".$self_protocol."' value='".$self_action_controller."'>".$self_action_controller."</option>";
										}
										foreach ($default_redirect_domain_list as $key => $value) 
										{
											echo "<option data-type='default' data-protocol='".$value["protocol"]."' value='".$value["action_controller_url"]."'>".$value["action_controller_url"]."</option>";
										}
										echo "</optgroup>";
									}
									if(!empty($custom_redirect_domain_list))
									{
										...
										{
											echo "<option data-type='custom' data-protocol='".$value["protocol"]."' value='".$value["action_controller_url"]."'>".$value["action_controller_url"]."</option>";
										}
										echo "</optgroup>";
									}
								echo "</select>";
							...
							<div class="box box-primary <?php echo $hide_social_block;?>">
								...
								<div class="box-body">
									<div class="account_list">
										...
										{											
											echo "<option value='rx_twitter_atuopost-".$value['id']."'>".$this->lang->line("Account").": ".$value['screen_name']."</option>";
							...
							<div class="box box-primary <?php echo $hide_social_block;?>">
								...
								<div class="box-body">
									...
									{											
										...
										{
											echo "<option value='rx_pinterest_info-".$info['table_id']."'>".$this->lang->line("Board").": ".$info['board_name']."</option>";
										}
										echo "</optgroup>";
							...
							<div class="box box-primary <?php echo $hide_social_block;?>" style="margin-top:34px;">
								...
								<div class="box-body">
									<div class="account_list">
										...
										{											
											echo "<option value='rx_linkedin_autopost-".$value['id']."'>".$this->lang->line("Account").": ". $value['first_name']." ".$value['last_name']."</option>";
							...
							<div class="box box-primary <?php echo $hide_social_block;?>" style="margin-top:21px;">
								...
								<div class="box-body">
									<div class="account_list">
										...
										{											
											echo "<option value='rx_tumblr_autopost-".$value['id']."'>".$this->lang->line("Account").": ".$value['user_name']."</option>";
					...
					<div class="form-group <?php echo $hide_pixel;?>">
						...
						<textarea class="form-control" name="pixel_code" id="pixel_code" placeholder="<?php echo $this->lang->line('paste your facebook pixel code here'); ?>"></textarea>
					</div>


					<div class="clearfix"></div>
					<div class="box-footer" style="padding:15px 0">
						<button class="btn btn-primary btn-lg" id="submit_post" name="submit_post" type="button"><i class="fa fa-send"></i> <?php echo $this->lang->line("create campaign"); ?></button>
					</div>
					<br>
...
<script>
	...
	$j("document").ready(function(){
		...
		var data_type2=$("#action_controller_url option:selected").attr("data-type");
		var data_protocol2=$("#action_controller_url option:selected").attr("data-protocol");		
		if(data_type2!='default')
		{
			$("#protocol").val(data_protocol2);
			$("#protocol_display").html(data_protocol2);		
			// $("#subdomain").val('').hide();
			// $("#dot").hide();
		}
		...
		var today = new Date();
		var next_date = new Date(today.getFullYear(), today.getMonth(), today.getDate()+60);
			...
			$("#response_modal").modal();
			  ...
			  $.ajax({
			   ...
			   {  		         
					...
					$("#submit_post").html('<i class="fa fa-send"></i> <?php echo $this->lang->line("create campaign")?>');    
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title"><i class="fa fa-send"></i> <?php echo $this->lang->line("create new campaign")?></h4>
			</div>
