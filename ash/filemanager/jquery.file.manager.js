...
var url_to_file_manager = 'http://jwel-pc/webadmin/filemanager/';
var categories = ["background", "image", "music","video","thumbnail"];
...
$(document).ready(function(){
    ...
    $(document).on('focus', 'input.file-manager', function () {
        ...
        var val = $(this).attr('location');

        var inArray = categories.indexOf(val);

        if(inArray == -1) {
            return false;
        }

        jQuery.post(url_to_file_manager + 'session_write.php', {value: val}); 
        jQuery.post(url_to_file_manager + 'session_write.php', {value: val});
        jQuery.post(url_to_file_manager + 'session_write.php', {value: val});       

        element = $(this);
        element_display = $(this).data('display-in');
    ...
    $(document).on('click', '#image-manager-insert', function () {
        ...
        if ($('#' + element_display).length > 0)
        {
            $('#' + element_display).empty().append('<img src="' + $('#image-manager-src').val() + '" />');
        }
    });
    ...
    function GetTheHtml(existing_files){
        ...
        html += '<iframe src="' + url_to_file_manager + 'image.php'+ '?' + new Date().getTime() + '&src=' + encodeURI(existing_files) + '&folder_name='+folder_name+'" frameborder="0" width="885" height="400"></iframe>';
