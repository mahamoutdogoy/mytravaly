$(document).ready(function(){
	...
	$(document).on('click', 'a.pdf-thumbs', function () {
		var file_surce = $(this).attr("rel");
		var ext = file_surce.split('.');
		ext = ext.pop();
		if(ext == "mp4" || ext == "wmv" || ext == "avi" || ext == "flv" || ext == "mkv" || ext == "MP4" || ext == "WMV" || ext == "AVI" || ext == "FLV" || ext == "MKV")
		{
			// var video_type = "video/"+ext;
			$("#videobox").show();
			$("#audiobox").hide();
			$("#preview").hide();

			$("#videosource").attr("src", $(this).attr("rel"));
			// $("#videosource").attr("type", video_type);
			$("#videobox").load();
		}
		...
		if(ext == "mp3")
		{
			...
			$("#audiosource").attr("src", $(this).attr("rel"));
			$("#audiobox").load();
		}
