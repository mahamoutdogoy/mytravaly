...
class Payment extends Home
{
    ...
    */
    ...
    public function admin_payment_history()
    {
        ...
        $data['body'] = 'admin/payment/admin_payment_history';
    ...
    {
        if ($this->session->userdata('user_type') != 'Admin' &&  $this->session->userdata('is_reseller') != '1' )
        redirect('home/login_page', 'location');
    ...
    public function package_data()
    {
       ...
       $allowed_package_ids=$this->session->userdata("allowed_package_ids");
       $allowed_package_ids=explode(',',$allowed_package_ids);
       if(empty($allowed_package_ids)) $allowed_package_ids=array(0);
       $where=array();
       if($this->session->userdata("user_type")!="Admin")
       $where=array("where_in"=>array("package.id"=>$allowed_package_ids));
        ...
        $offset = ($page-1)*$rows;
        $info=$this->basic->get_data('package',$where,$select=array("package.*","package.id as package_id"),$join='',$limit=$rows,$start=$offset,$order_by=$order_by_str,$group_by='',$num_rows=1);
    ...
    public function add_package_action()
    {
        ...
        if($_POST)
        {
            ...
            $this->form_validation->set_rules('validity', '<b>'.$this->lang->line("validity").'</b>', 'trim|required|integer');
    ...
    public function payment_dashboard_admin()
    {
        ...
        $data['body'] = 'admin/payment/payment_dashboard';

        $config_data=$this->basic->get_data("payment_config");
        $data["currency"]=$config_data[0]["currency"];

        $data['page_title'] = $this->lang->line("payment dashboard");
        $this->_viewcontroller($data);
    }


    public function add_custom_package_action()
    {
        if ($this->session->userdata('user_type') != 'Admin')
        redirect('home/login_page', 'location');
        ...
        if($_POST)
        {
            ...
            {
                $package_name = $package_name." + ".$value["package_name"];
                $modules_str = $modules_str.",".$value["module_ids"];
                $mod_lim = json_decode($value["monthly_limit"],true);
                $bul_lim = json_decode($value["bulk_limit"],true);

                foreach ($mod_lim as $key=>$value2)
                {
                    if(isset($monthly_limit[$key]) && ($monthly_limit[$key]>$value2 && $value2!="0")) continue;
                    $monthly_limit[$key] = $value2;
                }
                foreach ($bul_lim as $key=>$value2)
                {
                    if(isset($bulk_limit[$key]) && ($bulk_limit[$key]>$value2 && $value2!="0")) continue;
                    $bulk_limit[$key] = $value2;
                }
            ...
            $modules_str = trim($modules_str);
            $modules_str = trim($modules_str,",");
            $module_array =  explode(',',$modules_str);
            $module_array = array_unique($module_array);
            asort($module_array);
            $modules_str = implode(',',$module_array);
            ...
            // echo "modules_str :" .$modules_str."<br/>";
            // echo "monthly limit :";print_r($monthly_limit);echo "<br/>";
            // echo "bulk_limit :";print_r($bulk_limit);echo "<br/>";
            // exit();
    ...
    public function usage_history()
    {
        ...
        $info = $this->basic->get_data($table="modules", $where="", $select = "view_usage_log.*,modules.module_name,modules.id as module_id,extra_text",$join=array('view_usage_log'=>"view_usage_log.module_id=modules.id AND user_id =".$this->session->userdata("user_id").",left"),$limit='',$start=NULL,$order_by='module_name');
